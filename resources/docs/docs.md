...
The `home-routes` handler in the `clojure-jobs-luminus.routes.home` namespace
...
The `home-page` function will in turn call the `clojure-jobs-luminus.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `clojure-jobs-luminus.handler` namespace:
...
Request middleware functions are located under the `clojure-jobs-luminus.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `clojure-jobs-luminus.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `clojure-jobs-luminus.db.core` in some other namespace.
